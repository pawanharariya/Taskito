...
The testing code knows about the classes inside that are inside the main source set, so that it can
test those classes.
...
use `observeForever` on the LiveData and then    `removeObserver` to avoid observer leak. Do avoid
...
that runs before and after each run. This role run all architecture components related background
jobs in the same thread. This ensures that the test results happens synchronously and in a
repeatable order.
...
1. Fake - A test double that has a "working" implementation of the class, but it's implemented in a
   ...
   database using a HashMap implementation for testing purpose.
...
Service Locators - I some situations constructor dependency injection is not easily doable and
difficult to set up, such as injecting objects in activities and fragments. It is because there is
no easy way to change the constructors of activities and fragments. We use setter dependency
injection or the service locator pattern in such situations.

A service locator is a singleton whose purpose is to store and provide dependencies. It does it for
both the source code and for the test code. The benefit of using a service locator over setter
...
2. ViewMatcher - The ViewMatcher finds the required View in the UI. For example `withId()` find a
...
Mock : It is type of test double that knows whether its methods were called.

Avoiding misuse of mocks :
During testing we usually check that after doing some actions, the ending state is correct. For
example, calling a method and then see if some value is changed, or press a button and see if some
text changes.
...
1. Mock test doesn't check actual state change. For example if the `updateText()`method is broken,
   if gets called but doesn't update the text. The mock test still passes.

2. Mock version of test is more implementation dependent. If the code changes such we we no longer
...
to navigate to, is loaded up with correct details, as it is outside of current fragment test. We
only check the correct navigation method was called with correct arguments. We mock the navigation
...
1. Asynchronous code is non-deterministic - If we run operation A and operation B, run in parallel,
...
`runTest` is a coroutine builder designed for testing. It ensures that the test blocks until all of
the coroutines it starts are finished. It runs the coroutines in a deterministic order. It uses a
special `UnconfinedTestDispatcher` to run and suspend functions in `runTest`.

## Writing coroutine tests that use viewModelScope ##

ViewModel coroutines, be default use Dispatchers.Main because often ViewModels manipulate the UI.
...
If we ever switch dispatchers in the code under test, for example using Dispatchers.IO in
repository.
So, if the IO dispatcher starts a different dispatcher from the test dispatcher, the coroutines are
no longer run deterministically and blocking fashion. So we should avoid hard-coding the dispatcher
and instead use dependency injection to pass in the dispatcher. So that we can easily swap in the
test dispatcher for our tests.

JUnit Rules - These are classes where we define generic testing code that can execute before, after
or during tests. It is generally the code in `@Before` and `@After` blocks, put into a class where
...
For example, checking the state of loading indicator, before the coroutine starts and after it
...
In such cases the test double should be implemented return an error. For fakes, we can create an
...
5. When we test fragment and use `launchFragmentInContainer`, it launches the fragment in empty
...
7. Different Android devices are shipped with different versions of SQLite, so the Dao Unit tests
   should be run as instrumented tests on different devices. 
